...
export class AudioEngine {
	 ...
	 * Only safe to call in the browser (guarded by callers or `browser` check).
	 */
	getContext(): AudioContext {
		if (!this.context) {
			if (typeof AudioContext === 'undefined') {
				throw new Error('AudioContext is not available (SSR or unsupported browser)');
			}
	...
	playSample(
		...
		// Use addEventListener so multiple handlers can coexist (e.g. mute group cleanup)
