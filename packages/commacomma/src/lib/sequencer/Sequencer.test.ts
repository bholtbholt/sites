...
describe('Sequencer', () => {
	...
	describe('setOnPadTrigger', () => {
		it('should invoke callback when a step with an active pad is reached', () => {
	...
	describe('onStep', () => {
		it('should invoke step callback during playback', () => {
			if (typeof globalThis.window === 'undefined') {
				(globalThis as unknown as { window: typeof globalThis }).window = globalThis;
			}
			vi.useFakeTimers();
	...
	describe('dispose', () => {
		...
		it('should clear callbacks', () => {
			...
			playStateCallback.mockClear();
			sequencer.play();

			expect(playStateCallback).not.toHaveBeenCalled();
		});
	});
});
